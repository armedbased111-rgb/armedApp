...
echo "📋 Récupération des tracks disponibles..."
echo ""
RESPONSE=$(curl -s -X GET "http://localhost:3000/tracks" \
  -H "Authorization: Bearer $TOKEN")

echo "$RESPONSE" | jq '.'

echo ""
echo "📋 Format simplifié:"
echo "$RESPONSE" | jq 'if type == "array" then .[] | {id: .id, name: .name, projectId: .projectId} else . end'
